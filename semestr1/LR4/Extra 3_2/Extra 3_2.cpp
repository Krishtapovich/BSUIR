...
int _tmain() {
	...
	int p = n / 2;
	for (int z = 1; z <= p; z++) {
		for (int j = z - 1; j < n - z + 1; j++) { // верхняя строка
			sp[z - 1][j] = k++;
		}
		for (int i = z; i < n - z + 1; i++) {
			sp[i][n - z] = k++; // правый столбец
		}
		for (int j = n - z - 1; j >= z - 1; --j) {
			sp[n - z][j] = k++; // нижняя строка
		}
		for (int i = n - z - 1; i >= z; i--) {
			sp[i][z - 1] = k++; // левый столбец
		}
	}
	if (n % 2 != 0) {
		sp[p][p] = n * n;
		// заполнение центрального элемента в случае нечетности размера
	}
