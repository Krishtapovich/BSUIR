...
#pragma hdrstop
#include "Rectan.h"
void Rectan :: Set(int x1, int y1, int x2, int y2)
{
...
void Rectan :: Draw(TCanvas *Canvas)
{
...
void Rectan :: Resize(TCanvas *Canvas, int a)
{
	Hide(Canvas);
	if(a == 0)
	{
...
void Rectan :: MoveMC(TCanvas *Canvas, int x, int y)
{
	Hide(Canvas);
	if(massx != x)
	{
...
void Rectan :: DxDy(int x, int y)
{
	MassCenter();
	dx = (x - massx)/10;
	dy = (y - massy)/10;
}
void Rectan :: Rotate(TCanvas *Canvas, double ang)
{
	HideRotate(Canvas);
		if(angle <= ang)
		{
...
void Rectan:: HideRotate(TCanvas *Canvas)
{
	Canvas->Pen->Color = clWhite;
	Canvas->Pen->Width = 3;
	points[0] = Point(new_x1, new_y1);
